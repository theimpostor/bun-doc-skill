...
| `--filter <pattern>`, `-F`     | Only list dependencies of the matching workspaces ([filter syntax](/docs/pm/filter)) |
...
From a workspace root, Bun lists every workspace's dependencies. From inside a workspace package, Bun lists only that package's dependencies. Use [`bun why`](/docs/pm/cli/why) to find out what pulls in an unexpected package.
