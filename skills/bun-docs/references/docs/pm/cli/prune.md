# bun prune
Source: https://bun.com/docs/pm/cli/prune
...
Prune only the selected workspaces' `node_modules` folders (same patterns as [`bun install --filter`](/docs/pm/filter)). Bun also cleans shared locations: the root `node_modules`, or `node_modules/.bun` with the isolated linker. In those locations, Bun keeps anything an unselected workspace still needs.
...
* To clean the global cache instead, use [`bun pm cache rm`](/docs/pm/cli/pm#cache).
