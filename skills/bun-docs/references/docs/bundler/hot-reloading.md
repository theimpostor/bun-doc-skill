...
> Warning: This callback is not called on route navigation or when the browser tab closes.
...
> Note: For compatibility with Vite, the above events are also available via `vite:*` prefix instead of `bun:*`.
