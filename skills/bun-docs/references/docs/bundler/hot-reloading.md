...
> Note: HMR is enabled by default when using Bun's full-stack development server.
...
> Tip: Prefer `import.meta.hot.accept()` without an argument; it usually makes your code clearer.
...
> Warning: Bun does not call this callback on route navigation or when the browser tab closes.
...
> Note: For compatibility with Vite, these events are also available with the `vite:*` prefix instead of `bun:*`.
