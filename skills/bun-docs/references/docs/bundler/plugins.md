...
> Warning: You can call the `.defer()` function only once per `onLoad` callback.
...
> Info: Bun can call this callback from any thread, so the NAPI module implementation must be thread-safe.
