...
> Note: In the bundler, Bun handles `.node` files with the `file` loader.
...
> Warning: The `sqlite` loader is only supported when the target is `bun`.
...
> Note: The value of `naming.asset` determines the location and file name of the copied file.
