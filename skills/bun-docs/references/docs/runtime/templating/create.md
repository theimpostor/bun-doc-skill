...
> Note: By default Bun does *not overwrite* existing files. Use the `--force` flag to overwrite them.
...
> Note: Set the `BUN_CREATE_DIR` environment variable to change the global template path.
